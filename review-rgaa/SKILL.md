...
| aucun argument | **inférer** comme review-code : changements non commités → branche vs `main` → dernier commit |
...
5. **Titres de page** : vérifier `METADATA` dans `ui/utils/routes.metadata.utils.ts` (registre server-only, cf. #5214 ; `routes.utils.ts` n'en garde qu'un commentaire de renvoi) ou la constante `metadata` du `page.tsx`, quand une page est ajoutée ou renommée.
...
grep -nE '<(TextField|Input|OutlinedInput|Select|TextareaAutosize|input|textarea|select)\b' <fichiers> | grep -vE 'label=|aria-label|aria-labelledby|type="hidden"'
...
Les étapes ci-dessous sont des appels aux outils du navigateur intégré de Claude Code (`preview_start`, `read_page`, `javascript_tool`, `computer`, `resize_window`), pas des commandes shell. `ui-dev` et `server-dev` sont les configurations de `.claude/launch.json` du repo LBA. Si la session n'expose pas ces outils, passer directement au paragraphe « Incertain » en fin de section.
...
4. **axe-core** : la CSP de LBA (`ui/next.config.mjs`, `script-src`, appliquée à toutes les routes en dev comme en prod) n'autorise aucun CDN. Ne jamais injecter une URL externe, l'injection échoue systématiquement. Deux voies qui fonctionnent réellement, toutes deux avec `axe-core` en devDependency de `ui` (il n'est pas installé : le proposer, ne pas l'ajouter sans accord) :
   - **Servir axe depuis `ui/public`** (`'self'` est autorisé) : copier `node_modules/axe-core/axe.min.js` (racine ou `ui/`) vers `ui/public/__axe.min.js`, injecter, puis **supprimer le fichier** avant toute fin de session. Il ne doit jamais être commité.

```js
await new Promise((ok, ko) => { const s = document.createElement('script'); s.src = '/__axe.min.js'; s.onload = ok; s.onerror = ko; document.head.appendChild(s) })
...
```
   ...
   - **Playwright**, déjà présent dans `ui` (`@playwright/test`, script `test:e2e`) : un script ponctuel avec `@axe-core/playwright` (devDependency à ajouter aussi) qui ouvre la route et appelle `new AxeBuilder({ page }).withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa', 'wcag22aa']).analyze()`. Le script n'est pas commité.

   Si aucune voie n'est disponible dans la session : le dire et s'en tenir à `read_page` et aux contrôles `javascript_tool` ci-dessus. axe ne remplace pas la revue : il ne détecte ni les intitulés non pertinents, ni la gestion du focus, ni les titres simulés.

5. **Clavier** : `computer` avec `action: "key"`, `text: "Tab"` en répétition, puis `computer` avec `action: "screenshot"` : le focus est-il visible (10.7), l'ordre suit-il la lecture (12.8), Échap ferme-t-il modale et tooltip (7.3, 10.13), le focus revient-il sur le déclencheur ?
